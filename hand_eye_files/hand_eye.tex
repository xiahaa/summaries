...
\draw[-, line width=1mm, solid] (A1) to[] node[left,rotate=0] {} (B1);
...
\draw (B1) node[left, above] {$\bold{B}_2$};
...
\draw (B2) node[above] {$\bold{B}_n$};

%\draw[loosely dotted, color=black] (A) -- (A1);
\draw[<->, dashed] (A) to[bend left=80] node[above,rotate=0] {$\mathbf{T}_{\mathbf{A}_1}^{{\mathbf{A}}_2}$} (A1);
\draw[<->, dashed] (A1) to[bend left=80] node[above,rotate=0] {$\mathbf{T}_{\mathbf{A}_2}^{{\mathbf{A}}_n}$} (A2);
\draw[<->, dashed] (B) to[bend right=80] node[below,rotate=0] {$\mathbf{T}_{\mathbf{B}_1}^{{\mathbf{B}}_2}$} (B1);
\draw[<->, dashed] (B1) to[bend right=80] node[below,rotate=0] {$\mathbf{T}_{\mathbf{B}_2}^{{\mathbf{B}}_n}$} (B2);
...
\end{tikzpicture}
\caption{Coordinate Systems used for hand eye calibration. With out lossing generality, $\mathbf{A}$ can denote the camera frame with  $\mathbf{B}$ being the IMU body frame, which turns out to be the camera-IMU calibration problem. Note here the dashed connections are transformation matrices directly measured and dotted connections are transformation matrices to be optimized. The black thick solid line represents the rigid joint between two sensors.}
\label{fig:frame}
\end{figure}
